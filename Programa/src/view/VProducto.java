...
public class VProducto extends JInternalFrame implements ActionListener, IProducto {
	...
	private JTextField tcod, tnom, tpre;
	...
	public VProducto() {
		...
		precio.setFont(new Font("Arial Round TM Bold", Font.BOLD + Font.ITALIC, 20));
		precio.setForeground(white);
		precio.setBackground(darkblue);
		reglasde.gridy = 3;
		reglasde.gridx = 0;
		reglasde.fill = GridBagConstraints.HORIZONTAL;

		pde.add(precio, reglasde);

		tpre = new JTextField(4);
		tpre.setBackground(white);
		tpre.setForeground(Color.BLACK);
		tpre.setFont(new Font("Arial Round TM Bold", Font.BOLD, 16));
		reglasde.gridy = 3;
		reglasde.gridx = 1;
		tpre.setEditable(false);
		pde.add(tpre, reglasde);
		...
		lupa.setActionCommand(IProducto.BUSCAR);
		aceptar.setActionCommand(IProducto.ACEPTAR);
		limpiar.setActionCommand(IProducto.LIMPIAR);
		exit.setActionCommand(IProducto.EXIT);
		salir.setActionCommand(IProducto.SALIR);
	...
	public double getPrecio() {
		...
		try {
			precio = Double.parseDouble(tpre.getText());
			if(precio<=0) {
				MensajeError("DEBE INGRESAR UN 	COSTO VALIDO");
				validez = false;
				precio = 0;
			}
			else {
			...
			MensajeError("DEBE INGRESAR UN PRECIO VALIDO");
			validez = false;
			precio = 0;
		}
