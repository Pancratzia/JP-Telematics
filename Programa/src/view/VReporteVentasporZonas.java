...
public class VReporteVentasporZonas extends JInternalFrame implements ActionListener, IReporteVentasporZonas {
	...
	private CReporteVentasporZonas controlador;
	private boolean cerrado = true;
	private String sql = "SELECT ventas.hora_y_fecha, ventas.total, zonas.nombre_zona "
			+ "FROM ventas, clientes, vendedores, zonas WHERE " + "ventas.id_cliente = clientes.id_cliente AND "
			+ "clientes.id_vendedor = vendedores.id_vendedor " + "AND vendedores.id_zona = zonas.id_zona ";

	public VReporteVentasporZonas() {
		...
		JLabel titulo = new JLabel("LISTADO DE VENTAS POR ZONA"); // Titulo
		...
		String[] titulos = { "Hora y Fecha", "Total de la venta", "Nombre de la zona" }; // Titulos del Reporte
		String[][] data = {};
	...
	public void desplegar() {
		// TODO Auto-generated method stub
		controlador.actionPerformed(new ActionEvent(this, 1, IReporteVentasporZonas.IMPRIMIR));
		...
		try {
			if (resul.first()) {
				...
				while (resul.next()) {
					...
					filas[1] = Double.parseDouble(resul.getString(2));
